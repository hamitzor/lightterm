...
class OutputParser {
   ...
   parse(output) {
      if (this._previousOutput != output) {
         if (output.length > 10) {
         ...
         for (let i = 0; i < output.length; i++) {
            ...
            if (osc && symbol === '\u0007') {
               ...
               util.log('OSC ', value)
               this._context.addOSCommandData([controlSequenceKind.CHANGE_WINDOW_TITLE_ESCAPE, value])
               continue
            }
               ...
               if (match = new RegExp(/^\u001b\[(\d*)(?:;?(\d*))*m$/gmu).exec(symbol)) {
                  ...
                  styleCodes.forEach(code => {
                     if (code === 1) {
                        ...
                        globalStyleCodes[0] = 401
                        globalStyleCodes[1] = 402
                        globalStyleCodes[2] = 403
                        globalStyleCodes[3] = 400
                        globalStyleCodes[4] = 405
                     }
                  })
